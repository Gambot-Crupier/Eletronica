...
def getContinueStart():
    ...
    while r.status_code != 200:
        print('Não foi 200, foi ' + r.staus_code)
...
def postplayerid():
    ...
    payload = {"player_id": -1}
    r = requests.post("https://6wiv4418b6.execute-api.sa-east-1.amazonaws.com/production/post_player_id", json = payload) #Colocar o URL da galera de software aqui
...
def enviarmesaFlop(matriz):
    ...
    payload = dict()
    payload = {'cards': [{ 'value' : matriz[0][1], 'suit' : matriz[0][0]}, {'value' : matriz[1][1], 'suit' : matriz[1][0]}, { 'value' : matriz[2][1], 'suit' : matriz[2][0]}] }
...
def enviarmesaRT(matriz):
    ...
    payload = dict()
    payload = {'cards': [{ 'value' : matriz[0][1], 'suit' : matriz[0][0]}] }
    r = requests.post("https://6wiv4418b6.execute-api.sa-east-1.amazonaws.com/production/post_table_cards", json = payload) #Colocar o URL da galera de software aqui
    print(r.text) #pode apagar esse print
...
def enviarmao(matriz):
    ...
    for x in range(NumerodeJogadores):
        payload[x] = {'player_id': matriz[x][0], 'cards': [{ 'value' : matriz[x][2], 'suit' : matriz[x][1]}, {'value' : matriz[x][4], 'suit' : matriz[x][3]}] }
    r = requests.post("https://6wiv4418b6.execute-api.sa-east-1.amazonaws.com/production/post_hands", json = payload) #Colocar o URL da galera de software aqui
    print(r.text) #pode apagar esse print

def enviarmao2():
    payload = [dict() for x in range(2)]
    for x in range(NumerodeJogadores):
        payload[x] = {'player_id': "1", 'cards': [{ 'value' : "2", 'suit' : "c"}, {'value' : "4", 'suit' : "h"}] }
    r = requests.post("https://6wiv4418b6.execute-api.sa-east-1.amazonaws.com/production/post_hands", json = payload) #Colocar o URL da galera de software aqui
    print(r.text) #pode apagar esse print
...
def Posicionador(direcao):  # POSICIONADOR DA CARTA SOBRE A CÂMERA
    ...
    for i in range(200):  #1340 #2048 #708      
        pulso(pos_stp);
    GPIO.output(pos_ena, GPIO.HIGH)
...
def identificaPessoascomApp(salvaPosicao):
    ...
    for i in range(len(salvaPosicao)):
        ...
        if lejson == "-5":
            ...
            PosicaoDosJogadores.append(lerIdJogador)
